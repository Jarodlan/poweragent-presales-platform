...
1. 将需求分析报告一键发送给解决方案生成智能体
2. 将“待确认问题”转化为下一轮沟通提纲
3. 将行业标签、主题、约束作为场景路由输入
...
1. 前端通过 WebSocket / WebRTC / 流式 HTTP 上传音频分片
2. Django 平台层调用 `ASR Adapter`
3. `ASR Adapter` 路由到 `Qwen` 或 `FunASR`
4. ASR 服务返回临时文本
5. 后端做文本分段缓存
6. 对稳定文本做语义校验
...
2. 前端按固定时长切片
3. 上传到 Django 音频分片接口
4. Django 经 `ASR Adapter` 调用具体 ASR 实现
5. 后端回传临时文本与校验文本
...
#### 路线 A：Qwen ASR（MVP 先走 `qwen3-asr-flash`）
...
1. 先使用 `qwen3-asr-flash` 的 OpenAI compatible HTTP 接口处理短音频分片
2. 通过 Base64 Data URL 上传音频块，适合浏览器分片录音场景
3. 后续如需更低时延，再升级到 `Qwen3-ASR-Flash-Realtime` WebSocket 模式
